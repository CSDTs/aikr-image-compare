...
import getImages, { IImgurImage } from "./getImages";
...
class Search extends React.Component<IProps, IState> {
  ...
  public train = (e: React.MouseEvent<HTMLElement>) => {
    ...
    const evalImages = transformImages(
      this.state.picked,
      this.state.searches,
      5,
      10
  ...
  public handleRemoveTopicImage = (topic: string) => (id: string) => () => {
    this.setState({
      picked: {
        ...this.state.picked,
        [topic]: (this.state.picked[topic] || []).reduce(
          (pickeds: string[], picked) => {
            if (picked === id) {
              return pickeds;
            }
...
const transformImages = (
  ...
  return Object.entries(picked).reduce((collected, [label, pickedImages]) => {
    return collected.concat(
      pickedImages.slice(start, end).map((imageId: string) => {
        const image = searches[label].filter(
          ({ id }: IImgurImage) => id === imageId
        )[0];
