...
import { IImgurImage } from "./getImages";
...
const splitImages = (images: IImgurImage[], picked: string[]) => {
  ...
  return images.reduce(
    ({ pickedImages, unpickedImages }, image: IImgurImage) => {
      ...
      return {
        pickedImages,
        unpickedImages: [...unpickedImages, image],
      };
    },
    startingData
  );
...
const Images = ({
  ...
  const { pickedImages, unpickedImages } = splitImages(images, picked);
...
class ImageSet extends React.Component<IImageSetProps> {
  ...
  public render() {
    const { images, title, handleRemove } = this.props;
    ...
    return (
      <div className={className}>
        <label>
          {title} ({images.length}/5)
        </label>
        {images.map((image: IImgurImage) => (
          <SearchImage src={image.link} key={image.id}>
